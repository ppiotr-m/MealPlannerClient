...
class CuisineCustomizationFragmentTest {
    /* IMPORTANT!
   ...
   * */
            ...
            when (originOfNavigation) {
                ConstantValues.USER_PREFERENCES_FRAGMENT -> {
                    navController.setCurrentDestination(
                        R.id.cuisineCustomizationFragmentInUserPreferencesGraph
                    )
                }
                ConstantValues.PERSONALIZATION_PROCESS -> {
                    navController.setCurrentDestination(
                        R.id.cuisineCustomizationFragmentInPersonalizationProcess
                    )
                }
                else -> {
    ...
    private fun launchUserPreferencesFragment(originOfNavigationValue: String) {
        launchFragmentInHiltContainer<CuisineCustomizationFragment>(
    ...
    @Test
    fun openFragment_displayAllCuisinesNames() {
        ...
        Espresso.onView(ViewMatchers.withId(R.id.linearLayoutCuisineButtons))
            .check(ViewAssertions.matches(ViewMatchers.hasMinimumChildCount(3)))
    }

    @Test
    fun openFragment_correctCuisineTypeIsChecked() {
        setupNavController(ConstantValues.USER_PREFERENCES_FRAGMENT)
        launchUserPreferencesFragment(ConstantValues.USER_PREFERENCES_FRAGMENT)
        Espresso.onView(ViewMatchers.withText("ITALIAN"))
            .check(ViewAssertions.matches(ViewMatchers.isChecked()))
    }

    @Test
    fun clickCuisineType_updatedCuisineTypesPreference() {
        setupNavController(ConstantValues.USER_PREFERENCES_FRAGMENT)
        launchUserPreferencesFragment(ConstantValues.USER_PREFERENCES_FRAGMENT)
        Espresso.onView(ViewMatchers.withText("MEXICAN"))
        ...
        ViewMatchers.assertThat(
            testViewModel.getUserCuisines()?.contains("Mexican"),
    ...
    fun clickOnNextButton_navigatedToNextStepInPersonalization() {
        ...
        Espresso.onView(ViewMatchers.withId(R.id.confirmBtn)).perform(ViewActions.click())
        Truth.assertThat(navController.currentDestination?.id)
            .isEqualTo(R.id.mealsNumberCustomizationFragmentInPersonalizationProcess)
    }
}