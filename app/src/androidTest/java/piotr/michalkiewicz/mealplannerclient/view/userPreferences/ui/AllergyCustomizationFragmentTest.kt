...
class AllergyCustomizationFragmentTest {
    /* IMPORTANT!
   ...
   * */
            ...
            when (originOfNavigation) {
                ConstantValues.USER_PREFERENCES_FRAGMENT -> {
                    navController.setCurrentDestination(
                        R.id.allergyCustomizationFragmentInUserPreferencesGraph
                    )
                }
                ConstantValues.PERSONALIZATION_PROCESS -> {
                    navController.setCurrentDestination(
                        R.id.allergyCustomizationFragmentInPersonalizationProcess
                    )
                }
                else -> {
    ...
    private fun launchUserPreferencesFragment(originOfNavigationValue: String) {
        launchFragmentInHiltContainer<AllergyCustomizationFragment>(
    ...
    @Test
    fun openFragment_displayAllAllergiesNames() {
        ...
        Espresso.onView(ViewMatchers.withId(R.id.allergiesLinearLayout))
            .check(ViewAssertions.matches(ViewMatchers.hasMinimumChildCount(8)))
    }

    @Test
    fun openFragment_correctAllergyIsChecked() {
        setupNavController(ConstantValues.USER_PREFERENCES_FRAGMENT)
        launchUserPreferencesFragment(ConstantValues.USER_PREFERENCES_FRAGMENT)
        Espresso.onView(ViewMatchers.withText("PEANUTS"))
            .check(ViewAssertions.matches(ViewMatchers.isChecked()))
    }

    @Test
    fun clickAllergyButton_updatedAllergyPreference() {
        setupNavController(ConstantValues.USER_PREFERENCES_FRAGMENT)
        launchUserPreferencesFragment(ConstantValues.USER_PREFERENCES_FRAGMENT)
        Espresso.onView(ViewMatchers.withText("GLUTEN"))
        ...
        ViewMatchers.assertThat(
            testViewModel.getUserAllergies()?.contains("Gluten"),
